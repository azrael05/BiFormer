...
def read_data(root: str):
    ...
    for cl in os.listdir(root + '/train'):
        for im_path in os.listdir(root + '/train/' + cl):
            pt = root + '/train/' + cl
            train_images_path.append(os.path.join(pt,im_path))
            train_images_label.append(class_indices[cl])
    
    for cl in os.listdir(root + '/val'):
        for im_path in os.listdir(root + '/val/' + cl):
            pt = root + '/val/' + cl
...
class SupervisedContrastiveLoss(keras.losses.Loss):
    ...
    def __call__(self, labels, feature_vectors, sample_weight=None):
        ...
        feature_vectors_normalized = torch.nn.functional.normalize(feature_vectors, dim=1, p=2)
        # Compute logits
        logits = torch.divide(
            torch.matmul(
                feature_vectors_normalized, torch.transpose(feature_vectors_normalized)
            ),
            self.temperature,
        )
        return tfa.losses.npairs_loss(torch.squeeze(labels), logits)