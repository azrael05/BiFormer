...
class BiFormer(nn.Module):
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=2, embed_dim=[64, 128, 320, 512],
        ...
        if representation_size:
         ...
         ## Contrastive Layers
        self.contrastive_hidden_layer = nn.Linear(embed_dim[-1], contrastive_dimension)
    ...
    def freeze_projection(self):
        selected_layers =['contrastive_hidden_layer', 'contrastive_output_layer', 'head']
        for name, param in self.named_parameters():
            if not any(layer_name in name for layer_name in selected_layers):
